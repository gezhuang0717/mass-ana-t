...
// ----------------------------------------------------------------------------
#ifndef BOOST_PROPERTY_TREE_DETAIL_YAON_PARSER_READ_HPP_INCLUDED
#define BOOST_PROPERTY_TREE_DETAIL_YAON_PARSER_READ_HPP_INCLUDED
...
namespace boost { namespace property_tree { namespace yaon_parser
{
    ...
    struct context
    {
        ...
        struct a_decimal_val
        {
            context &c;
            a_decimal_val(context &c): c(c) { }
    ...
    template<class Ptree>
    struct yaon_grammar :
        public boost::spirit::classic::grammar<yaon_grammar<Ptree> >
    {
        ...
        struct definition
        {
            ...
            boost::spirit::classic::rule<Scanner>
                root, object, member, array, item, value, string,
                decimal, number;
            boost::spirit::classic::rule<
                typename boost::spirit::classic::lexeme_scanner<Scanner>::type>
                character, escape;

            definition(const yaon_grammar &self)
            {
			...
			| decimal[typename Context::a_decimal_val(self.c)]
    ...
    template<class Ptree>
    void read_yaon_internal(std::basic_istream<typename Ptree::key_type::value_type> &stream,
                            Ptree &pt,
                            const std::string &filename)
    {
        ...
        if (!stream.good())
            BOOST_PROPERTY_TREE_THROW(yaon_parser_error("read error", filename, 0));

        // Prepare grammar
        yaon_grammar<Ptree> g;
        ...
        {
            BOOST_PROPERTY_TREE_THROW(yaon_parser_error(e.descriptor, filename, count_lines<It, Ch>(v.begin(), e.where)));
        }
