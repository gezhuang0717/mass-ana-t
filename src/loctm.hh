...
class loc_tm : public tm
{
  ...
  public:
	...
	loc_tm (const std::string time_str_ISO); //format: 2005-04-04 12:13:14

    loc_tm & operator= (tm const & t);            // Assignment
	loc_tm & operator= (std::string time_str_ISO);	 //Assignment from string


    operator time_t (void) const;        // Conversion operator
    int is_valid (void) const;             // Test for validity
//	std::string getAverage(const loc_tm &a,const loc_tm &b);
	std::string AverageWith(const loc_tm &a);
			std::string get_time_as_string();
			std::string get_just_time_as_string();
    ...
    enum date_f                         // Composite date flags
    {
        ...
        d_Full          = (int)(d_DMY|d_DayOfWeek|d_MonText|
                                d_SepSpace),
		d_oma			= (int)(d_DMY|d_YearFirst|d_SepChar|d_PadDMY|d_ZeroDMY|d_MonFirst
								)
    };
    ...
    static int timefmt;                  // Default time format
                                              ...
                                              // Output methods
    std::ostream & print (std::ostream & os, int df =datefmt,
    ...
    std::istream & parseTime (std::istream & is);        // Unimplemented
    std::istream & parseDate (std::istream & is);        // Unimplemented
    std::istream & parse     (std::istream & is);        // Unimplemented
...
inline std::ostream &                        // Stream output method
operator<< (std::ostream & os, loc_tm const & t)
{   return t.print(os);     }
...
inline std::ostream &                // Stream output method - date
operator<< (std::ostream & os, loc_date const & d)
{   return d.print(os);     }
...
inline std::ostream &                // Stream output method - time
operator<< (std::ostream & os, loc_time const & t)
{   return t.print(os);     }



//getAverage(const loc_tm &a, const loc_tm &b);
# endif     // _loctm_h
